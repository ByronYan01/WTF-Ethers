 ...
 *
 * 1.减少交易数量
 * 批量转账：所有转账合并为 单笔交易，只需支付一次基础 gas 费用
 * 2.数据压缩优化
 * 地址数组和金额数组在 calldata 中存储更紧凑
 * 相同前缀的地址可压缩（如 0x71C2... 开头）
 * 相同金额只需存储一次（示例中所有金额相同）
 */
...
const main = async () => {
  ...
  if (
    ...
    // 发起交易
    // 第三个参数是覆盖选项；若 value < ∑amounts，交易会因余额不足失败，若 value > ∑amounts，多余 ETH 会滞留合约（需设计退款逻辑）
